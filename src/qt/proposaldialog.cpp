...
ProposalDialog::ProposalDialog(Mode mode, QWidget* parent) : QDialog(parent), ui(new Ui::ProposalDialog), mapper(0), mode(mode), counter(0)
{
    ...
    ui->addressEdit->setPlaceholderText(tr("The public HTH address that will receive the funds"));
    ui->addressEdit->setToolTip(tr("The public HTH address that will receive the funds"));
...
void ProposalDialog::prepareProposal()
{
    ...
    CScript scriptPubKey = GetScriptForDestination(ExtractDestination(address));
...
bool ProposalDialog::validateProposal()
{
    ...
    std::string address = ui->addressEdit->text().toStdString();
    if (!IsValidDestinationString(address)) strError = "Invalid HTH address";
