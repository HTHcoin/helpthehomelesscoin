...
ProposalDialog::ProposalDialog(Mode mode, QWidget* parent) : QDialog(parent), ui(new Ui::ProposalDialog), mapper(0), mode(mode), counter(0)
{
    ...
    ui->addressEdit->setPlaceholderText(tr("The public ODIN address that will receive the funds"));
    ui->addressEdit->setToolTip(tr("The public ODIN address that will receive the funds"));
...
bool ProposalDialog::validateProposal()
{
    ...
    int nBlockMin = 0;
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (pindexPrev != NULL) nBlockMin = pindexPrev->nHeight - pindexPrev->nHeight % consensusParams.nBudgetPaymentsCycleBlocks + consensusParams.nBudgetPaymentsCycleBlocks;

    int nBlockStart = ui->blockEdit->text().toInt();
    if (nBlockStart < nBlockMin) strError = "Invalid block start, must be more than current height.";
    if (nBlockStart % consensusParams.nBudgetPaymentsCycleBlocks != 0)
    {
        int nNext = pindexPrev->nHeight - pindexPrev->nHeight % consensusParams.nBudgetPaymentsCycleBlocks + consensusParams.nBudgetPaymentsCycleBlocks;
        strError = strprintf("Invalid block start - must be a budget cycle block. Next valid block: %d", nNext);
    }

    int nBlockEnd = nBlockStart + (consensusParams.nBudgetPaymentsCycleBlocks * nPaymentCount); // End must be AFTER current cycle
    ...
    std::string address = ui->addressEdit->text().toStdString();
    if (!IsValidDestinationString(address)) strError = "Invalid ODIN address";
