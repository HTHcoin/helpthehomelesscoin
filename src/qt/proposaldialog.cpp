...
ProposalDialog::ProposalDialog(Mode mode, QWidget* parent) : QDialog(parent), ui(new Ui::ProposalDialog), mapper(0), mode(mode), counter(0)
{
    ...
    int nNext = pindexPrev->nHeight - pindexPrev->nHeight % GetNearestSuperblocksHeights() + GetNearestSuperblocksHeights();
...
bool ProposalDialog::validateProposal()
{
    ...
    int nBlockMin = 0;
    CBlockIndex* pindexPrev = chainActive.Tip();
    if (pindexPrev != NULL) nBlockMin = pindexPrev->nHeight - pindexPrev->nHeight % GetNearestSuperblocksHeights() + GetNearestSuperblocksHeights();

    int nBlockStart = ui->blockEdit->text().toInt();
    if (nBlockStart < nBlockMin) strError = "Invalid block start, must be more than current height.";
    if (nBlockStart % GetNearestSuperblocksHeights() != 0)
    {
        int nNext = pindexPrev->nHeight - pindexPrev->nHeight % GetNearestSuperblocksHeights() + GetNearestSuperblocksHeights();
        strError = strprintf("Invalid block start - must be a budget cycle block. Next valid block: %d", nNext);
    }

    int nBlockEnd = nBlockStart + (GetNearestSuperblocksHeights() * nPaymentCount); // End must be AFTER current cycle
