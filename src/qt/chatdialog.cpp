...
ChatDialog::ChatDialog(const PlatformStyle *platformStyle, QWidget *parent) :
    QDialog(parent),
...
void ChatDialog::setModel(WalletModel *model)
{
    this->model = model;
    ...
    if(model && model->getOptionsModel())
    {
        ...
        {
            ChatEntry *entry = qobject_cast<ChatEntry*>(ui->entries->itemAt(i)->widget());
            if(entry)
            {
...
void ChatDialog::send(QList<SendCoinsRecipient> recipients, QString strFee, QString strFunds)
{
...
{
    ChatEntry *entry = new ChatEntry(platformStyle, this);
    entry->setModel(model);
    ui->entries->addWidget(entry);
    connect(entry, SIGNAL(removeEntry(ChatEntry*)), this, SLOT(removeEntry(ChatEntry*)));
...
void ChatDialog::updateTabsAndLabels()
{
    setupTabChain(0);
    coinControlUpdateLabels();
}

void ChatDialog::removeEntry(ChatEntry* entry)
{
    entry->hide();
...
QWidget *ChatDialog::setupTabChain(QWidget *prev)
{
    for(int i = 0; i < ui->entries->count(); ++i)
    {
        ChatEntry *entry = qobject_cast<ChatEntry*>(ui->entries->itemAt(i)->widget());
...
void ChatDialog::setAddress(const QString &address)
{
    ...
    if(ui->entries->count() == 1)
    {
        ChatEntry *first = qobject_cast<ChatEntry*>(ui->entries->itemAt(0)->widget());
        if(first->isClear())
        {
            entry = first;
        }
...
bool ChatDialog::handlePaymentRequest(const SendCoinsRecipient &rv)
{
...
void ChatDialog::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance,
...
void ChatDialog::updateDisplayUnit()
{
    setBalance(model->getBalance(), model->getUnconfirmedBalance(), model->getImmatureBalance(), model->getAnonymizedBalance(),
                   model->getWatchBalance(), model->getWatchUnconfirmedBalance(), model->getWatchImmatureBalance());
...
void ChatDialog::updateInstantSend()
{
    QSettings settings;
...
void ChatDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
...
void ChatDialog::minimizeFeeSection(bool fMinimize)
{
...
void ChatDialog::on_buttonChooseFee_clicked()
{
    minimizeFeeSection(false);
}

void ChatDialog::on_buttonMinimizeFee_clicked()
{
    updateFeeMinimizedLabel();
    minimizeFeeSection(true);
}

void ChatDialog::setMinimumFee()
{
    ui->radioCustomPerKilobyte->setChecked(true);
    ui->customFee->setValue(CWallet::GetRequiredFee(1000));
}

void ChatDialog::updateFeeSectionControls()
{
...
void ChatDialog::updateGlobalFeeVariables()
{
...
void ChatDialog::updateFeeMinimizedLabel()
{
    if(!model || !model->getOptionsModel())
        return;
...
void ChatDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
...
void ChatDialog::updateSmartFeeLabel()
{
    if(!model || !model->getOptionsModel())
        return;
...
void ChatDialog::coinControlClipboardQuantity()
{
    GUIUtil::setClipboard(ui->labelCoinControlQuantity->text());
}

// Coin Control: copy label "Amount" to clipboard
void ChatDialog::coinControlClipboardAmount()
{
    GUIUtil::setClipboard(ui->labelCoinControlAmount->text().left(ui->labelCoinControlAmount->text().indexOf(" ")));
}

// Coin Control: copy label "Fee" to clipboard
void ChatDialog::coinControlClipboardFee()
{
    GUIUtil::setClipboard(ui->labelCoinControlFee->text().left(ui->labelCoinControlFee->text().indexOf(" ")).replace(ASYMP_UTF8, ""));
}

// Coin Control: copy label "After fee" to clipboard
void ChatDialog::coinControlClipboardAfterFee()
{
    GUIUtil::setClipboard(ui->labelCoinControlAfterFee->text().left(ui->labelCoinControlAfterFee->text().indexOf(" ")).replace(ASYMP_UTF8, ""));
}

// Coin Control: copy label "Bytes" to clipboard
void ChatDialog::coinControlClipboardBytes()
{
    GUIUtil::setClipboard(ui->labelCoinControlBytes->text().replace(ASYMP_UTF8, ""));
}

// Coin Control: copy label "Dust" to clipboard
void ChatDialog::coinControlClipboardLowOutput()
{
    GUIUtil::setClipboard(ui->labelCoinControlLowOutput->text());
}

// Coin Control: copy label "Change" to clipboard
void ChatDialog::coinControlClipboardChange()
{
    GUIUtil::setClipboard(ui->labelCoinControlChange->text().left(ui->labelCoinControlChange->text().indexOf(" ")).replace(ASYMP_UTF8, ""));
}

// Coin Control: settings menu - coin control enabled/disabled by user
void ChatDialog::coinControlFeatureChanged(bool checked)
{
...
void ChatDialog::coinControlButtonClicked()
{
...
void ChatDialog::coinControlChangeChecked(int state)
{
...
void ChatDialog::coinControlChangeEdited(const QString& text)
{
...
void ChatDialog::coinControlUpdateLabels()
{
    if (!model || !model->getOptionsModel())
        return;
