...
define $(package)_preprocess_cmds
  sed -i 's/add_executable(runtest test.cpp)/#add_executable(runtest test.cpp)/' src/CMakeLists.txt && \
  sed -i 's/add_executable(runbench test-bench.cpp)/#add_executable(runbench test-bench.cpp)/' src/CMakeLists.txt && \
  sed -i 's/target_link_libraries(runtest/#target_link_libraries(runtest/' src/CMakeLists.txt && \
  sed -i 's/target_link_libraries(runbench/#target_link_libraries(runbench/' src/CMakeLists.txt
endef
...
define $(package)_set_vars
  ...
  $(package)_config_opts_mingw32=-DOPSYS=WINDOWS -DCMAKE_SYSTEM_NAME=Windows -DCMAKE_SHARED_LIBRARY_LINK_C_FLAGS="" -DCMAKE_SHARED_LIBRARY_LINK_CXX_FLAGS="" -DBUILD_BLS_TESTS=OFF -DBUILD_BLS_BENCHMARKS=OFF
